...
export default function Home() {
  ...
  const [editingData, setEditingData] = useState({ quantidade: 0, valor_unitario: 0 });
  ...
  const excluirInvestimento = async (id: number) => {
    try {
      ...
      console.error("Erro ao excluir investimento:", error.response?.data || error.message);
        ...
        {editingId === inv.id ? (
          <div className="editar-bloco">
            <input
              ...
              min={0}
              onChange={(e) => setEditingData({ ...editingData, quantidade: Number(e.target.value) })}
            />
            <input
              type="number"
              ...
              min={0}
              step={0.01}
              onChange={(e) => setEditingData({ ...editingData, valor_unitario: Number(e.target.value) })}
            />
            <button className='salvar' onClick={() => salvarEdicao(inv.id)}>Salvar</button>
            <button className='cancelar' onClick={cancelarEdicao}>Cancelar</button>
          </div>
        ) : (
          <div className='icons'>
            <button className='editar' onClick={() => iniciarEdicao(inv)}>Editar</button>
            <button className='excluir' onClick={() => excluirInvestimento(inv.id)}>Excluir</button>
          </div>
        )}
      </li>
    ))}
